...
const Shopcart = () => {
  // imported states
  const { idState } = useOutletContext(); // jeremy: all useOutletContext state can be combined in one desctructure
  ...
  //jeremy: const { idState: [userId, setUserId], productsState: [products, setProducts], etc. etc.} = useOutletContext()
  ...
  async function fetchUpdatedCartItems() {
    try {
      const response = await fetch(
        `https://project-09-backend.onrender.com/api/shopcart/${userId}/status`,
        {
          method: "POST",
          headers: {
  ...
  async function changeProductQuantity(event) {
    ...
    try {
      const response = await fetch(
        `https://project-09-backend.onrender.com/api/shopcart/${shopCartId}/quantity`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      const data = await response.json(); //jeremy: if you don't use it, lose it
  ...
  async function removeProductFromOrder(event) {
    ...
    try {
      const response = await fetch(
        `https://project-09-backend.onrender.com/api/shopcart/${shopCartId}/remove`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  async function checkOutFunc(event) {
    ...
    try {
      const response = await fetch(
        `https://project-09-backend.onrender.com/api/shopcart/${shopCartId}/status`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
        ...
        <div>
          ...
          {pendingOrders && !!pendingOrders.length
            ? pendingOrders.map((order, idx) => {
                {
                  // jeremy: no need to have this in {}
                  totalNum += order.quantity * order.priceBoughtAt;
                }

                // jeremy: this can all go in a component:
                // pendingOrders.map((order, idx) => <MatchingProduct key={idx} order={order} />)
          ...
          <div>
            <br />
            <p>
              Shipping and handling: <b>FREE!</b>
            </p>
            <b>Total: ${totalNum.toFixed(2)}</b>
          </div>
          {/* jeremy: START this can be a component */}
          <div>
            <form onSubmit={checkOutFunc}>
              Enter shipping info
              <div id="address-form">
          ...
          {/* jeremy: END this can be a component */}
          {newError && !!newError.length ? (
            <div>
              <p>{newError}</p>
            </div>
          ) : null}
          {/* jeremy: if the above is null, you can simply do: newError && !!newError.length && <div>etc... */}
