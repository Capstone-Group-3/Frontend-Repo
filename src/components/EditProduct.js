...
const EditProduct = ({ indivProduct }) => {
  ...
  async function deactivateProduct(event, prodId) {
    ...
    await setTargetedProduct(prodId); // jeremy: no await needed here
  ...
  async function deleteProduct(event, id) {
    ...
    try {
      const response = await fetch(
        `https://project-09-backend.onrender.com/api/products/${id}`,
        {
          method: "DELETE",
          headers: {
  ...
  async function editProduct(event, targetedProduct) {
    ...
    try {
      const response = await fetch(
        `https://project-09-backend.onrender.com/api/products/${targetedProduct}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
        ...
        } // jeremy: nice use of productInfo object
      );
  ...
  return (
    <div className="adminIndivEdit">
      <h3>{name}</h3>
      {isActive ? <h5>Active item</h5> : <h5>Inactive item</h5>}
      <form onSubmit={(e) => editTargetProduct(e, id)}>
        <label>Name</label>
        <br />
        <input
          value={targetName}
          onChange={(e) => handleInputChange(e, setTargetName)}
        ></input>
        <br />
        ...
        <input
          ...
          onChange={(e) => handleInputChange(e, setTargetDescription)}
        ></input>
        <br />
        <label>Price</label>
        <br />
        ...
        <input
          ...
          onChange={(e) => handleInputChange(e, setTargetQuantity)}
        ></input>
        <br />
        ...
        <button
          type="button"
          onClick={(e) => deactivateProduct(e, id)}
          value={id}
        >
          Delete
        </button>
