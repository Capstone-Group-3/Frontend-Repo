...
const AdminPanel = () => {
  ...
  // ---------------Homepage States-------------
  const { usernameState, adminState, currentToken, productsState } =
    useOutletContext();
  ...
  async function adminUser() {
    try {
      ...
      // jeremy: if unused, lose it.
    } catch (error) {
      console.error;
      // jeremy: find these and change them all to console.error(error)
    }
  }
  ...
  async function makeProduct() {
    ...
    try {
      const response = await fetch(
        `https://project-09-backend.onrender.com/api/products`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      // if statement here or get rid of data if unused
    } catch (error) {
      console.error(error);
    }
  }
    ...
    async function getRegUsers() {
      try {
        const userGroup = await fetch(
          "https://project-09-backend.onrender.com/api/users/nonAdmin",
          {
            headers: {
              "Content-Type": "application.json",
              Authorization: `Bearer ${currentToken}`,
  ...
  // jeremy: any time you have a map, it's an opportunity to make a component
  return (
    <div>
      {isAdmin ? (
        <div>
          ...
          <button
            ...
            onClick={() => setIsUsersToggled(!isUsersToggled)}
            value="Show Users"
          />
          <div className="userContainer">
            {isUsersToggled &&
              regUsers.map((indivUser, idx) => {
                return (
                  <form
                    className="adminIndivUser"
                    onSubmit={promoteUser}
                    key={idx}
                  >
                    <p classname="IndivUserChild">{indivUser.username}</p>
                    <button
                      classname="IndivUserChild userButton"
                      type="submit"
                      value={indivUser.name}
                    >
                      Promote
                    </button>
                  </form>
          ...
          <button
            className="openButton"
            onClick={() => setIsProductsToggled(!isProductsToggled)}
          >
            Show Products
          </button>
          <div className="editProductContainer">
            {isProductsToggled &&
              products.map(
                // jeremy: productsMapCallback
                (indivProduct, idx) => (
                  <EditProduct key={idx} indivProduct={indivProduct} />
                )
              )}
          </div>
          <div>
            ...
            <div className="makeProduct">
              <form className="adminIndivEdit" onSubmit={makeProduct}>
                <label>Name</label>
                <br />
                {/* jeremy: can do this in styling of inputs "margin-top: 10px" */}
                <input
                  value={addName}
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange(e, setAddDescription)}
                ></input>
                <br />
                <label>Price</label>
                <br />
                <input
                  value={addPrice}
                  onChange={(e) => handleInputChange(e, setAddPrice)}
                ></input>
                <br />
                ...
                <input
                  value={addQuantity}
                  onChange={(e) => handleInputChange(e, setAddQuantity)}
                ></input>
                <br />
