...
const App = () => {
  ...
  useEffect(() => {
    async function loadProfileInfo() {
      try {
        const response = await fetch(
          "https://project-09-backend.onrender.com/api/users/me",
          {
            headers: {
              "Content-Type": "application/json",
  ...
  useEffect(() => {
    ...
    async function loadCurrentShoppingCart() {
      try {
        const response = await fetch(
          "https://project-09-backend.onrender.com/api/shopcart/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div>
      <div>
        ...
        <Navbar
          pendingOrdersState={[pendingOrders, setPendingOrders]}
          isAdmin={isAdmin}
          loggedIn={loggedIn}
