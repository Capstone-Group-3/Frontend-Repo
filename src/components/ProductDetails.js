...
const ProductDetails = () => {
  ...
  const [toggleProductDetailsForm, setToggleProductDetailsForm] =
    useState(false);
  ...
  useEffect(() => {
    function findSpecificProduct() {
      const [specificProduct] = products.filter((element) => element.id == id); // jeremy: can use array.find(callback) here, but fine as is.
  ...
  async function addProductToCart(event) {
    ...
    try {
      const response = await fetch(
        `https://project-09-backend.onrender.com/api/shopcart/${shopCartId}/add`,
        {
          method: "PATCH",
          headers: {
      ...
      <div>
        {detailedSpecificProduct.isActive ? ( // extra check in case someone uses a route to go to an inactive product
          <div class="details">
            ...
            <img
              src={detailedSpecificProduct.image}
              alt="product-image"
              height="300"
              width="300"
            />
            {currentToken && !!currentToken.length ? ( // jeremy: can use && after !!currentToken.length and remove ternary :null
            ...
            {successMessage && !!successMessage.length ? ( // jeremy: can use && after !!currentToken.length and remove ternary :null
              <p>{successMessage}</p>
            ) : null}
            <br></br>{" "}
